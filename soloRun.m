...
for itr = 1:AlgorithmParams.NumOfDays
    for ii=1:AlgorithmParams.NumOfShares
        ...
        elseif(itr>RSITimeFrame && Shares(ii).RSI(itr-1)>70)
            [Shares, local, AlgorithmParams]= ModifiedFalling(ii,Shares, local, AlgorithmParams,ProblemParams,bestSolution,globalCost,itr,alpha);
        else
            r=rand;
            if(r>0.5)
                [Shares, AlgorithmParams]= ModifiedRising(ii,Shares,AlgorithmParams,ProblemParams,bestSolution,itr, alpha);
    ...
    if(mod(itr, 20) == 0)        
        randomNumber = 0.1 + rand * (0.4 - 0.1);
        [Shares, globalCost] = DumpAndPump(Shares, AlgorithmParams, ProblemParams, globalCost, ProblemParams.ub, ProblemParams.lb,randomNumber);
    end
    if(mod(itr, 20) == 0)  
        randomNumber = 0.4 + rand * (0.8 - 0.4);
