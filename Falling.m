function [Shares, local, AlgorithmParams] = Falling(ii,Shares,local, AlgorithmParams,ProblemParams, BestShare,itr)
    ...
    Shares(ii).Position = (Shares(ii).Position -d).* AlgorithmParams.NegativeCoefficient .* rand(size(d));
