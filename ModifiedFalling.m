function [Shares, local, AlgorithmParams] = ModifiedFalling(ii, Shares, local, AlgorithmParams, ProblemParams, BestShare,BestCost, itr,alpha)
    ...
    randomFactor = (alpha) * rand() * 2;
    Shares(ii).Position = (Shares(ii).Position -d).* AlgorithmParams.NegativeCoefficient * randomFactor .* rand(size(d));
